...
#------------------------------------------------------------------
#compare_punctuation function
#-------------------------------------------------------------------
#Registers used:
# $a3=holds the row of token to be checked, parameter t
# $v1=return value
# $t1=row
# $t2=previous
# $t3=next
# $t4=elipsis
# $t5=token_num/ascii code for comparison/mul factor
# $t6=index of tokens big array
# $t8=second char in token checked               
        
         
           
compare_punctuation:
      move $t1,$a3                      #int row=t;
      li $t2,0                          #char previous='\0';
      li $t3,0                          #char next='\0';
      li $t4,0                          #int elipsis=0; //flag for elipsis
      li $t8,0                          #next element in token
      
      li $t5,201
      mul $t6,$t1,$t5
      addi $t6,$t6,1     
      lb $t8,tokens($t6)                #get the next element 
      
      bnez $t1,get_previous             #if (row!=0){
      
second_check:      
      lw $t5,token_num                  #if (row!=tokens_number){
      bne $t1,$t5,get_next
third_check:      
      move $t6,$t1
      li $t5,201   
      ...
      li $t5,46
      beq $t7,$t5,get_elipsis           #if (tokens[row][0]=='.'){
      j rules
...
get_previous:
      ...
      li $t5,201
      mul $t6,$t6,$t5
      lb $t2,tokens($t6)                #previous=tokens[t-1][0];
      j second_check
get_next:
      move $t6,$t1
      addi $t6,$t6,1
      li $t5,201
      mul $t6,$t6,$t5
      lb $t3,tokens($t6)                #next=tokens[t+1][0];
      j third_check
...
get_elipsis:
      ...
      li $t5,46
      lb $t7,tokens($t6)
      bne $t7,$t5,rules                 #if (tokens[row][1]=='.' 
      addi $t6,$t6,1                    # &&
      lb $t7,tokens($t6) 
      bne $t7,$t5,rules                 #tokens[row][2]=='.'
      addi $t6,$t6,1                    #&&
      lb $t7,tokens($t6) 
      bnez $t7,rules                    #tokens[row][3]=='\0'
      li $t4,1                          #elipsis=1;
      j rules

rules:
      li $t5,32
      beq $t2,$t5,incorrect             #if (previous==32){
      li $t5,65
      bge $t3,$t5,incorrect             #else if (next>=65){
      li $t5,1 
      beq $t4,$t5,correct               #else if (elipsis==1){
      beqz $t8,correct                  #else if (tokens[row][1]=='\0'){
      bnez $t8,incorrect                #else if (tokens[row][1]!='\0'){
      j incorrect                       #return 0 if any of the if-statements not entered
...
not_alpha:
    li $t0,32
    bne $t5,$t0,punctuation_check        #check if char is space or punctuation
    j print_token                       #if space, then just print token
...
punctuation_check:
    ...
    jal compare_punctuation
    
    lw $ra,0($sp)                       # get register back
    addi $sp,$sp,4                      #increament pointer
    
    move $t7,$v1                        #  test=compare_punctuation(rt);
    li $t0,1
    bne $t7,$t0,incorrect_print_pun       # if (test!=1){
    j print_token
